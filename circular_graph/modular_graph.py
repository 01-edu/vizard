...
class modular_graph:
    def __init__(self,graph_json,data, piscines_list, checkpoints_list, mandatory_list, gradient_colors=None):
        # defs of the svg
...
###############################################################################################
    ...
    def get_arc_bounding_angles(self,arcs_types, index, gap, ref_arc, rotate):
        ...
        slices_count = arcs_types.count('slice') # Assumes 'line' takes 0 space for length calculation
        ...
        slice_length = (ref_arc_length - gaps_angle) / slices_count if slices_count > 0 else 0
        arcs_lengths = [slice_length if arc_type == 'slice' else 0 for arc_type in arcs_types]

        base_start_angle = rotate - (arcs_lengths[0] / 2 if arcs_lengths else 0) if ref_arc_is_circle and not single_arc else ref_arc["startAngle"] + rotate
...
###############################################################################################
    ...
    def get_arc_coords(self,center_coords, radius, arcs_types, index, reverse=False, gap=0, rotate=0, ref_arc=None):
        """
        ...
        start_angle, end_angle, mid_angle = angles["startAngle"], angles["endAngle"], angles["midAngle"]
        ...
        large_arc_flag = 1 if on_circle_lower_edge or (end_angle - start_angle > 180) else 0
        ...
        start = self.polar_to_cartesian(center_coords["x"], center_coords["y"], radius, start_angle_flipped)
        end = self.polar_to_cartesian(center_coords["x"], center_coords["y"], radius, end_angle_flipped)
        middle = self.polar_to_cartesian(center_coords["x"], center_coords["y"], radius, mid_angle_flipped)

        d_str = f"M {start['x']} {start['y']} A {radius} {radius} 0 {large_arc_flag} {sweep_flag} {end['x']} {end['y']}{end_command}"
        return {"path": d_str, "start": start, "end": end, "middle": middle, "fullCircle": full_circle}
...
###############################################################################################
    ...
    def render_star_icon(self,x, y, fill, width, name,content_name,value):
        ...
        """
        g = self.create_element("g", {"transform": f"translate({x - width / 2}, {y - width / 2}) scale({width/130})"})
        title = self.create_element("title", text_content="Star icon")
        path = self.create_element("path", {
            "fill": fill,
...
###############################################################################################
    ...
    def render_checkpoint_icon(self,x, y, fill, width):
        ...
        g = self.create_element("g", {"transform": f"translate({x - width / 2}, {y - height_factor / 2}) scale({width/18})"}) 
...
###############################################################################################
    ...
    def render_text_path(self,parent_group, text, id_str, index, circle_params):
        ...
        path_el = self.create_element("path", {"d": arc_path_data["path"], "id": id_str, "fill": "none"})
        g_text_path.append(path_el)

        text_el = self.create_element("text", {"fill": circle_params.get("nameColor", self.COLORS["neutral"]),
                                        "text-anchor":"middle",
                                        "dominant-baseline":"middle",
                                        "font-size": str(circle_params.get("nameFontSize", 50)),
                                        "font-family": circle_params.get("nameFont", "IBM Plex Sans")})
        text_path_el = self.create_element("textPath", {
            "startOffset": f"{50}%",
            "xlink:href": f"#{id_str}",
...
###############################################################################################
    ...
    def render_content(self,parent_group, content_item_data, circle_props_from_parent, object_attrs=None, is_sub_content=False,content_name = None):
        """
        ...
        if object_attrs is None: object_attrs = {}
        
        name = self.get_content_name(content_item_data)        
        value = self.data.get(name, 0)
        

        coords = self.polar_to_cartesian(self.CURRENT_CENTER, self.CURRENT_CENTER, circle_props_from_parent["radius"], circle_props_from_parent["angle"])
        ...
        fill_color = self.COLORS["neutral"] if value == 0 else value_to_color(value, self.max_value, self.gradient_colors) 
        ...
        icon_radius = (self.PISCINE_CONSTANTS["radius"] if is_piscine 
                    else circle_props_from_parent.get("contentRadius", 4))

        if name in self.checkpoints_list:
            icon = self.render_checkpoint_icon(x, y, fill_color, self.CHECKPOINT_CONSTANTS["subContentWidth"] if is_sub_content else self.CHECKPOINT_CONSTANTS["width"])
            content_group.append(icon)
        elif obj_mandatory:
            icon = self.render_star_icon(x, y, fill_color, self.STAR_CONSTANTS["subContentWidth"] if is_sub_content else self.STAR_CONSTANTS["width"], name, name if not content_name else content_name, value)
            content_group.append(icon)
        else:
            if is_piscine:
                gradient = self.create_element("radialGradient",attributes={"id":content_item_data+"_gradient",
                                                                    "cx":"50%",
                                                                    "cy":"50%",
                                                                    "r":"50%",
                                                                    "fx":"50%",
                                                                    "fy":"50%"})
                gradient.append(self.create_element("stop",attributes={"offset":"0%","stop-color":fill_color,"stop-opacity":"1"}))
                gradient.append(self.create_element("stop",attributes={"offset":"40%","stop-color":fill_color,"stop-opacity":"0.5"}))
                gradient.append(self.create_element("stop",attributes={"offset":"100%","stop-color":fill_color,"stop-opacity":"0"}))
                self.defs.append(gradient)
            
            circle_el = self.create_element("circle", {
                "fill": fill_color if not is_piscine else f"url(#{content_item_data}_gradient)",
                "r": str(icon_radius),
                "cx": str(x),
                "cy": str(y),
                "id": name,
        ...
        # Content name text
        name_offset = (self.PISCINE_CONSTANTS["nameOffset"] if is_piscine
                    else circle_props_from_parent.get("contentNameOffset", 34))
        if not is_sub_content:
            text_el = self.create_element("text", {
                "x": str(x), "y": str(y + name_offset),
                "font-size": "12px", "text-anchor": "middle", "fill": self.COLORS["neutral"],
                "font-family": "IBM Plex Mono", "style": "text-transform: uppercase;"
            }, text_content=name) 
            content_group.append(text_el)

        if has_sub_contents:
            self.render_sub_contents(content_group, content_item_data, circle_props_from_parent)

################################################################################################
################################################################################################
    
    # component rendering function for sub_content
    def render_sub_contents(self,parent_g, content_data_with_subs, parent_circle_props):
        """
        ...
        if not sub_contents_list: return

        sub_radius = parent_circle_props["radius"] + self.SLICE_CONSTANTS["innerCircle"]["subContentRadiusOffset"]

        start_coords = self.polar_to_cartesian(self.CURRENT_CENTER, self.CURRENT_CENTER, parent_circle_props["radius"], parent_circle_props["angle"])
        end_coords = self.polar_to_cartesian(self.CURRENT_CENTER, self.CURRENT_CENTER, sub_radius, parent_circle_props["angle"])

        # Line connecting to sub-contents
        line_el = self.create_element("line", {
            "x1": str(start_coords["x"]), "y1": str(start_coords["y"]),
            "x2": str(end_coords["x"]), "y2": str(end_coords["y"]),
            "stroke": self.COLORS["neutral"], "stroke-width": "1", "opacity": "0.5"
            # Simplified: no gradient
        })
        parent_g.append(line_el)

        total_gap_offset = (len(sub_contents_list) - (len(sub_contents_list) % 2)) * self.SLICE_CONSTANTS["innerCircle"]["subContentGap"]
        ...
        for i, sub_name in enumerate(sub_contents_list):
            ...
            sub_circle_props = {
                **parent_circle_props, 
                "radius": sub_radius,
                "angle": sub_angle,
                "contentRadius": parent_circle_props.get("subContentRadius", self.SLICE_CONSTANTS["innerCircle"]["subContentRadius"]),
            }
            
            self.render_content(parent_g, sub_name, sub_circle_props, is_sub_content=True,content_name=None)

#####################################################################################################################################################
#####################################################################################################################################################

    # component rendering function for arc
    def render_arc(self,parent_group, section_data, circle_config_from_parent, index, id_prefix):
        """
        ...
        if not single_content:
            path_el = self.create_element("path", {
                "d": arc_coords_data["path"], "fill": "none",
                "stroke": self.COLORS["grey"], "stroke-width": "0.25"
            })
        ...
        if not section_name_data.get("hidden", False) and section_name_data.get("text"):
            ...
            self.render_text_path(arc_g, section_name_data["text"], text_path_id, index, text_path_circle_params)

        # Distribute contents
        chunk_divider = len(contents) - (0 if arc_coords_data["fullCircle"] else 1)
        if chunk_divider <= 0: chunk_divider = 1 # Avoid division by zero for single items
        ...
        if end_angle_norm < start_angle_norm and not arc_coords_data["fullCircle"] : # handles wrap around 360
            # this happens if arc crosses the 0/360 degree line
            if abs(end_angle_norm - start_angle_norm) > 180: # Large arc likely means it crossed 0
                end_angle_norm +=360

        arc_angle_length = end_angle_norm - start_angle_norm
        if arc_coords_data["fullCircle"]: arc_angle_length = 360
        
        chunk_angle = arc_angle_length / chunk_divider if chunk_divider > 0 else 0


        for i, content_item in enumerate(contents):
            content_angle = arc_coords_data["middle"]["angle"] if single_content else (start_angle_norm + chunk_angle * i)
            
            content_circle_props = {
                "radius": circle_config_from_parent["radius"], # Content sits on the main arc radius
                "angle": content_angle,
                "contentRadius": circle_config_from_parent.get("contentRadius", 5),
                "contentNameOffset": circle_config_from_parent.get("contentNameOffset", 34),
...
###############################################################################################################################
    ...
    def render_slice(self,parent_group, slice_data, index, all_sections_types):
        ...
        if not slice_name_data.get("hidden", False) and slice_name_data.get("text"):
            ...
            text_circle_params = {**self.SLICE_CONSTANTS["textCircle"], "arcs": all_sections_types}
            self.render_text_path(slice_g, slice_name_data["text"], text_path_id, index, text_circle_params)
        ...
        if entry_point_key:
            ...
            outer_arc_coords_for_entry = self.get_arc_coords(
                center_coords={"x": self.CURRENT_CENTER, "y": self.CURRENT_CENTER},
                radius=self.SLICE_CONSTANTS["outerCircle"]["radius"], # JS uses SLICE.outerCircle.radius
            ...
            self.render_content(slice_g, entry_point_key, entry_point_circle_props, object_attrs={"type": "piscine"})
        ...
        if inner_arc_data:
            ...
            self.render_arc(slice_g, inner_arc_data, inner_arc_circle_config, index, f"{slice_id}-inner")

        # Outer Arcs
        outer_arcs_data = slice_data.get("outerArcs", [])
        if outer_arcs_data:
            outer_arcs_types = ['slice'] * len(outer_arcs_data) # Outer arcs within a slice are always 'slice' type relative to each other
            ...
            if len(all_sections_types) == 1:
                ref_arc_for_outer_arcs = {"startAngle": self.SLICE_CONSTANTS["outerArc"]["gap"]/2, "endAngle": 360 - self.SLICE_CONSTANTS["outerArc"]["gap"]/2}
            ...
            for arc_idx, arc_data in enumerate(outer_arcs_data):
                ...
                self.render_arc(slice_g, arc_data, outer_arc_circle_config, arc_idx, f"{slice_id}-outer")
...
########################################################################################################################################################################
    ...
    def render_line_section(self,parent_group, line_data, index, all_sections_types):
        ...
        arc_coords_for_line_angle = self.get_arc_coords(
            center_coords={"x": self.CURRENT_CENTER, "y": self.CURRENT_CENTER},
            radius=self.SLICE_CONSTANTS["innerCircle"]["radius"], # Radius reference from JS for angle
            arcs_types=all_sections_types,
            gap=self.SLICE_CONSTANTS["innerCircle"]["gap"],
            index=index
        )
        line_angle = arc_coords_for_line_angle["start"]["angle"] # Use start angle for line orientation

        start_coords = self.polar_to_cartesian(self.CURRENT_CENTER, self.CURRENT_CENTER, self.LINE_CONSTANTS["startRadius"], line_angle)
        end_coords = self.polar_to_cartesian(self.CURRENT_CENTER, self.CURRENT_CENTER, self.LINE_CONSTANTS["endRadius"], line_angle)
        ...
        if not line_name_data.get("hidden", False) and line_name_data.get("text"):
            ...
            text_el = self.create_element("text", {
                "x": str(start_coords["x"]), "y": str(start_coords["y"]),
                "font-size": "21px", "fill": self.COLORS["neutral"], "font-family": "IBM Plex Mono",
                "alignment-baseline": "middle", "text-anchor": text_anchor,
        ...
        line_offset_for_content = 0 if line_name_data.get("hidden", True) else self.LINE_CONSTANTS["startOffset"]
        line_length_for_content = self.LINE_CONSTANTS["endRadius"] - self.LINE_CONSTANTS["startRadius"] - line_offset_for_content
        
        num_contents = len(contents)
        chunk_length = line_length_for_content / (num_contents -1 ) if num_contents > 1 else 0


        for i, content_item in enumerate(contents):
            radius_for_content = (self.LINE_CONSTANTS["endRadius"] if num_contents == 1
                                else self.LINE_CONSTANTS["startRadius"] + line_offset_for_content + chunk_length * i)
            content_circle_props = {
                "radius": radius_for_content,
                "angle": line_angle,
                "contentRadius": self.SLICE_CONSTANTS["innerCircle"]["contentRadius"], # Example from inner circle
                "contentNameOffset": self.SLICE_CONSTANTS["innerCircle"]["contentNameOffset"],
            }
...
###############################################################################################################################
    ...
    def render_circular_map01(self,graph_data):
        ...
        if central_point_key:
            ...
            cp_attrs = {"type": "exam"} # Defaulting to exam type as per JS 'Checkpoint' icon
            ...
            cp_circle_props = {
                ...
                "contentRadius": self.SLICE_CONSTANTS["centralPointCircle"]["contentRadius"],
                # Add other necessary props if render_content expects them, e.g., contentNameOffset
                "contentNameOffset": self.SLICE_CONSTANTS["centralPointCircle"].get("contentNameOffset", 34)
            }
            self.render_content(root, central_point_key, cp_circle_props, object_attrs=cp_attrs)
        ...
        if middle_circle_sections:
            ...
            middle_circle_config = {**self.MIDDLE_CIRCLE_CONSTANTS, "arcs": middle_sections_types}
            for i, section in enumerate(middle_circle_sections):
                self.render_arc(middle_g, section, middle_circle_config, i, "middle-circle")
        ...
        if outer_circle_sections:
            ...
            outer_circle_config = {**self.OUTER_CIRCLE_CONSTANTS, "arcs": outer_sections_types}
            for i, section in enumerate(outer_circle_sections):
                self.render_arc(outer_g, section, outer_circle_config, i, "outer-circle")
        ...
        info_card = self.create_element("g",{"id":"info_card","filter":"url(#filter8_d_1_272)","style":"visibility: hidden;"})
        root.append(info_card)
        card = self.create_element("g",{"id":"card"})
        info_card.append(card)
        card_a = self.create_element("rect",{"fill":"#9C9797","height":"69","id":"card_a","rx":"5", "width":"110", "x":"722", "y":"651"})
        card.append(card_a)
        card_b = self.create_element("rect",{"fill":"#A3A3A3","height":"68", "id":"card_b", "rx":"4.5", "stroke":"#656464", "width":"109", "x":"722.5", "y":"651.5"})
        card.append(card_b)
        text1 = self.create_element("text",{"fill":"white", "font-family":"Inter", "font-size":"12", "font-weight":"800", "letter-spacing":"0em", "style":"white-space: pre", "xml:space":"preserve"})
        info_card.append(text1)
        project_name_card = self.create_element("tspan",{"id":"project_name_card", "x":"737.535", "y":"666.864"},text_content="project name")
        text1.append(project_name_card)
        text2 = self.create_element("text",{"fill":"white", "font-family":"Inter", "font-size":"12", "font-weight":"800", "letter-spacing":"0em", "style":"white-space: pre", "xml:space":"preserve"})
        info_card.append(text2)
        data_card = self.create_element("tspan",{"id":"data_card", "x":"753.35", "y":"699.364"})
        ...
        filter = self.create_element("filter",{"color-interpolation-filters":"sRGB","filterunits":"userSpaceOnUse","height":"63" ,"id":"filter7_f_1_272", "width":"65", "x":"855", "y":"994"})
